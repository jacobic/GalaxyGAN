...
def roou():
    ...
    parser.add_argument(
        "--input",
        default="/u/jacobic/GalaxyGAN/data/fitsdata/fits_train")
    #./fits_0.01_0.02
    ...
    for i in files:
        ...
        # with problem
        figure_blurred = cv2.GaussianBlur(figure_original, (5, 5),
                                          gaussian_sigma)
        ...
        figure_original_nearzero = figure_original_nz[
            figure_original_nz > -0.1]
        ...
        whitenoise = np.random.normal(0, np.sqrt(whitenoise_var),
                                      (data_g.shape[0], data_g.shape[1]))
        ...
        figure_combined[:, data_g.shape[1]:2 *
                        data_g.shape[1], :] = figure_blurred[:, :, :]
        ...
        if mode == 2:
            figure_combined_no_ori = np.zeros(data_g.shape[0],
                                              data_g.shape[1] * 2, 3)
            figure_combined_no_ori[:, data_g.shape[
                1]:2 * data_g.shape[1], :] = figure_blurred[:, :, :]
